...
@XmlType(name = "LossInfoType", propOrder = { "causeOfLoss", "reportedDate", "lossDescription" })
public class LossInfoType {
	...
	/**
	 * Coverts the JAXB class to the model class Lossinfotype
	 * 
	 * @return Lossinfotype
	 */
	public Lossinfotype getLossInfo() {
		XMLGregorianCalendar date = this.getReportedDate();
		Date newDate = (date == null) ? null : date.toGregorianCalendar().getTime();
		return new Lossinfotype(null, this.getCauseOfLoss().value(), newDate, this.getLossDescription());
	}

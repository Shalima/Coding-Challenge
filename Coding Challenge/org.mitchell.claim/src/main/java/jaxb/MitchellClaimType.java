...
// Generated on: 2015.11.14 at 12:40:52 PM PST 
//
...
@XmlType(name = "MitchellClaimType", propOrder = { "claimNumber", "claimantFirstName", "claimantLastName", "status",
		"lossDate", "lossInfo", "assignedAdjusterID", "vehicles" })
public class MitchellClaimType {
	...
	/**
	 * Converts the JAXB class to the model class Claim
	 * 
	 * @return Claim
	 */
	public Claim getClaim() {
		XMLGregorianCalendar date = this.getLossDate();
		// check for null values and set accordingly
		Date newDate = (date == null) ? null : date.toGregorianCalendar().getTime();
		String newStatus = (this.status == null) ? null : this.status.value();
		String adjusterId = (this.assignedAdjusterID == null) ? null : this.assignedAdjusterID.toString();
		Lossinfotype lossInfo = (this.lossInfo == null) ? new Lossinfotype() : this.lossInfo.getLossInfo();
		List<Vehicleinfotype> vehicleList = (this.vehicles == null) ? null : this.vehicles.getVehicles();

		Claim claim = new Claim(this.getClaimNumber(), this.getClaimantFirstName(), this.getClaimantLastName(),
				newStatus, newDate, adjusterId, vehicleList, lossInfo);
		claim.getVehicleinfotypes().forEach((vehicle) -> vehicle.setClaim(claim));
		return claim;
	}
